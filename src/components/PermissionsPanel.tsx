...
import { LayoutDashboard, Activity, Settings, Check, Pencil, Save, Palette, ScrollText, ExternalLink, Shield } from 'lucide-react';
...
function PermissionsPanel({ companyId, permissions, onChange }: Props) {
  ...
  const hasProxyAdmin = permissions.some(p => p.permission_type === 'proxy_admin');
  ...
  const toggleProxyAdmin = () => {
    let next: PermissionEntry[];
    if (hasProxyAdmin) {
      next = permissions.filter(p => p.permission_type !== 'proxy_admin');
    } else {
      next = [...permissions, { permission_type: 'proxy_admin', resource_id: null, access_level: 'access' }];
    }
    onChange(next);
  };
        ...
        {activeCategory === 'settings' && (
          <div>
            ...
            <div className="p-3 border-b border-gray-200 dark:border-gray-600">
              ...
              >
                <div className="flex items-center gap-2">
                  <Shield className={`w-4 h-4 flex-shrink-0 ${hasProxyAdmin ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400'}`} />
                  <div className="text-left">
                    <span className={`text-sm font-medium block leading-tight ${hasProxyAdmin ? 'text-blue-700 dark:text-blue-400' : 'text-gray-700 dark:text-gray-300'}`}>
                      Manage Proxy Settings
                    </span>
                    <span className="text-xs text-gray-500 dark:text-gray-400 leading-tight">Toggle static IP proxy on endpoints</span>
                  </div>
                </div>
                <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ml-2 ${
                  hasProxyAdmin ? 'bg-blue-600' : 'border-2 border-gray-300 dark:border-gray-500'
                }`}>
                  {hasProxyAdmin && <Check className="w-3 h-3 text-white" />}
                </div>
              </button>
            </div>
